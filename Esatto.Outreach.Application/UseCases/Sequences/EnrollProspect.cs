...
public class EnrollProspect
{
    ...
    public EnrollProspect(ISequenceRepository sequenceRepo, IProspectRepository prospectRepo, SequenceAccess access)
    {
    ...
    public async Task<SequenceProspectViewDto> Handle(Guid sequenceId, EnrollProspectRequest request, string userId, CancellationToken ct = default)
    {
        ...
        var sequenceProspect = sequence.EnrollProspect(request.ProspectId, request.ContactPersonId);
