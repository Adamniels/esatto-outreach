...
namespace Esatto.Outreach.Infrastructure.OpenAI;

public sealed class OpenAIWebSearchClient : IOpenAIWebSearchClient
{
    ...
    public OpenAIWebSearchClient(HttpClient http, IOptions<OpenAiOptions> options)
    {
        _http = http;
        ...
        CancellationToken ct = default)
    {
        ...
        var payload = BuildPayload(
            ...
            temperature: 0.3, // Lägre för mer faktabaserade svar
            maxOutputTokens: _options.DefaultMaxOutputTokens
        );

        var outputText = await CallOpenAI(_options.ApiKey, payload, ct);

        // Parse JSON response till SoftCompanyDataDto
...
- Prioritera kvalitet över kvantitet";
        ...
        int maxOutputTokens)
    {
        ...
        return new Dictionary<string, object?>
        {
            ...
            ["tool_choice"] = "auto",
            ["tools"] = new object[] { new { type = "web_search" } } // Web search alltid aktiverad
        };
    }

    private async Task<string> CallOpenAI(string apiKey, object payload, CancellationToken ct)
    {
    ...
    private static SoftCompanyDataDto ParseResearchResponse(string outputText, string companyName)
    {
        ...
        {
            throw new InvalidOperationException($"OpenAI returnerade tomt svar för {companyName}");
        }
        ...
        // Hitta JSON-objektet i texten (om det finns omgivande text)
        ...
        try
        {
            // Parse till en temporary structure
